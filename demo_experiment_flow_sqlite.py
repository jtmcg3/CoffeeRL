#!/usr/bin/env python3
"""
Demo script to demonstrate the complete CoffeeRL experiment flow and data collection using SQLite.
...
def demonstrate_experiment_creation():
    ...
    return experiment.id, tracker


def demonstrate_user_interactions(experiment_id: int, tracker: ExperimentTracker):
...
def demonstrate_result_submission(experiment_id: int, tracker: ExperimentTracker):
...
def demonstrate_data_retrieval(experiment_id: int, tracker: ExperimentTracker):
...
def demonstrate_user_statistics(tracker: ExperimentTracker):
...
def demonstrate_database_inspection():
    """Show how to inspect the SQLite database directly."""
    print_section("7. DATABASE INSPECTION")

    print("You can inspect the SQLite database directly using:")
    print("\n📊 SQLITE COMMAND LINE:")
    print("   sqlite3 cofferl.db")
    print("   .tables")
    print("   .schema experiments")
    print("   SELECT * FROM experiments;")
    print("   SELECT * FROM experiment_results;")
    print("   SELECT * FROM user_interactions;")

    print("\n🔍 PYTHON INSPECTION:")
    print("   import sqlite3")
    print("   conn = sqlite3.connect('cofferl.db')")
    print("   cursor = conn.cursor()")
    print("   cursor.execute('SELECT * FROM experiments')")
    print("   print(cursor.fetchall())")


def main():
    """Run the complete demonstration."""
    print_section("COFFEERL EXPERIMENT FLOW DEMONSTRATION (SQLite)")
    print(
        "This demo shows the complete data collection and experiment flow using SQLite."
    )
    print("The Gradio interface is running at: http://127.0.0.1:7861")

    try:
        # Run through the complete flow
        experiment_id, tracker = demonstrate_experiment_creation()
        demonstrate_user_interactions(experiment_id, tracker)
        result_id = demonstrate_result_submission(experiment_id, tracker)
        demonstrate_data_retrieval(experiment_id, tracker)
        demonstrate_user_statistics(tracker)
        demonstrate_web_interface_flow()
        demonstrate_database_inspection()
