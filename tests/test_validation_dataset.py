...
class TestCategorizeByExtraction:
    ...
    def test_categorize_from_reasoning(self):
        ...
        examples = [
            {
                "output": {
                    "grind_change": "unknown",
                    "reasoning": "under-extraction detected",
                }
            },
            {
                "output": {
                    "grind_change": "unknown",
                    "reasoning": "over-extraction problem",
                }
            },
    ...
    def test_remove_duplicates(self):
        ...
        categories = {
            "good": [
                ...
                {
                    "input": "V60,20gcoffee,300gwater",
                    "output": {"confidence": 0.8},
                },  # Duplicate
                {
                    "input": "V60, 25g coffee, 350g water",
        ...
        for i in range(50):
            examples.extend(
                [
                    {
                        "input": f"under_{i}",
                        "output": {"extraction": "under", "confidence": 0.8},
                    },
                    {
                        "input": f"good_{i}",
                        "output": {"extraction": "good", "confidence": 0.8},
                    },
                    {
                        "input": f"over_{i}",
                        "output": {"extraction": "over", "confidence": 0.8},
                    },
                ]
            )

        validation, training = create_balanced_validation_set(
            examples, validation_size=99
        )
...
class TestSaveDatasets:
    ...
    def test_save_hf_datasets(self, tmp_path):
        ...
        val_dataset = Dataset.load_from_disk(
            str(tmp_path / "coffee_validation_dataset")
        )
        train_dataset = Dataset.load_from_disk(
            str(tmp_path / "coffee_training_dataset")
        )
...
class TestIntegration:
    ...
    def test_end_to_end_workflow(self, tmp_path):
        ...
        validation, training = create_balanced_validation_set(
            all_examples, validation_size=30
        )
