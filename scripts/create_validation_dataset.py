        ...
        for community_file in community_dir.glob("*_examples*.json"):
            with open(community_file, 'r') as f:
                ...
                if isinstance(community_data, list):
                    ...
                    print(f"Loaded {len(community_data)} examples from {community_file.name}")
    
    print(f"Total examples loaded: {len(all_examples)}")
    return all_examples


def categorize_by_extraction(examples: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
...
def select_high_quality_examples(categories: Dict[str, List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
...
def create_balanced_validation_set(
    ...
    for category, category_examples in hq_categories.items():
        ...
        print(f"Selected {len(selected)} examples from '{category}' category for validation")
    ...
    if remaining_needed > 0:
        ...
        remaining_hq = [ex for ex in high_quality_examples if id(ex) not in validation_ids]
        
        # Shuffle and take what we need
        random.shuffle(remaining_hq)
        validation_examples.extend(remaining_hq[:remaining_needed])
        print(f"Added {min(remaining_needed, len(remaining_hq))} additional examples to reach target size")
...
def main() -> None:
    ...
    validation_examples, training_examples = create_balanced_validation_set(all_examples)
